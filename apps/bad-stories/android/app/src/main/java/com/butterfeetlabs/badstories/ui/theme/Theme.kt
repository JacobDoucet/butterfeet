...
private val BadStoriesShapes = Shapes(
...
/**
 * Bad Stories design tokens exposed via [LocalBadStoriesTokens]. Holds families and
...
val LocalBadStoriesTokens = compositionLocalOf { BadStoriesTokens() }

object BadStories {
    val tokens: BadStoriesTokens
        @Composable
        @ReadOnlyComposable
        get() = LocalBadStoriesTokens.current
}
...
fun BadStoriesTheme(
    ...
    androidx.compose.runtime.CompositionLocalProvider(
        LocalBadStoriesTokens provides BadStoriesTokens()
    ) {
        MaterialTheme(
            colorScheme = colors,
            typography = BadStoriesTypography,
            shapes = BadStoriesShapes,
            content = content
        )
    }
}
