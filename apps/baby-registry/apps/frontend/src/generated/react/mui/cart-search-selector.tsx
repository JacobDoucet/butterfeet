...
import { CartWithRefs, CartSearchQuery } from '../../model/cart-api';
import { useSearchCarts } from '../tanstack-query/cart-queries';

export type CartSearchSelectorOptions = {
    label?: string;
    placeholder?: string;
    multiple?: boolean;
    disabled?: boolean;
    clearable?: boolean;
    searchProjection?: CartProjection;
    defaultSort?: CartSortParams;
    getOptionLabel: (option: CartWithRefs) => string;
    getOptionDescription?: (option: CartWithRefs) => string;
    isOptionEqualToValue?: (option: CartWithRefs, value: CartWithRefs) => boolean;
    filterOptions?: (options: CartWithRefs[], state: any) => CartWithRefs[];
    filter?: CartSearchQuery;
...
export type CartSearchSelectorProps = CartSearchSelectorOptions & {
    value?: CartWithRefs | CartWithRefs[] | null;
    onChange: (value: CartWithRefs | CartWithRefs[] | null) => void;
    onInputChange?: (inputValue: string) => void;
    size?: 'small' | 'medium';
};

export function useCartSearchSelector(options: CartSearchSelectorOptions) {
    ...
    // Fetch data
    const { data: searchResults, isLoading, error } = useSearchCarts({
        query: searchQuery,
    ...
    const autocompleteProps = useMemo(() => ({
        ...
        isOptionEqualToValue: options.isOptionEqualToValue || ((option, value) => option.cart.id === value.cart.id),
...
export type UseCartSearchSelectorResult = {
    autocompleteProps: ReturnType<typeof useCartSearchSelector>['autocompleteProps'];
    inputValue: string;
    setInputValue: (value: string) => void;
    searchResults: ReturnType<typeof useSearchCarts>['data'];
    isLoading: boolean;
    error: ReturnType<typeof useSearchCarts>['error'];
    options: CartWithRefs[];
};
...
export function useCartSearchSelectorPresets() {
    return {
        // Basic selector with minimal data
        minimal: (): CartSearchSelectorOptions => ({
            searchProjection: { id: true },
            getOptionLabel: (option) => option.cart.id || '',
            enableSearch: false,
        }),

        // Full-featured selector with search
        searchable: (searchProjection?: CartProjection): CartSearchSelectorOptions => ({
        ...
        multiple: (searchProjection?: CartProjection): CartSearchSelectorOptions => ({
            multiple: true,
            searchProjection: searchProjection || { id: true },
            getOptionLabel: (option) => option.cart.id || '',
            enableSearch: true,
...
// Main Cart selector component
export function CartSearchSelector(props: CartSearchSelectorProps) {
    const {
        value,
        onChange,
        onInputChange,
    ...
    return (
        <Autocomplete<CartWithRefs, boolean, boolean, boolean>
            {...autocompleteProps}
            sx={options.sx}
            size={props.size || 'small'}
            value={value}
            onChange={(_, newValue) => {
                onChange(newValue as CartWithRefs | CartWithRefs[] | null);
            }}
