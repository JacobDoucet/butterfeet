...
type AggregateResultRow struct {
	...
	AmountCents       *int                `bson:"amountCents" json:"amountCents,omitempty"`
	ClaimedAt         *time.Time          `bson:"claimedAt" json:"claimedAt,omitempty"`
	ContributorEmail  *string             `bson:"contributorEmail" json:"contributorEmail,omitempty"`
	ContributorName   *string             `bson:"contributorName" json:"contributorName,omitempty"`
	Currency          *string             `bson:"currency" json:"currency,omitempty"`
	DecidedAt         *time.Time          `bson:"decidedAt" json:"decidedAt,omitempty"`
	DecisionReason    *string             `bson:"decisionReason" json:"decisionReason,omitempty"`
	Message           *string             `bson:"message" json:"message,omitempty"`
	MethodDisplayName *string             `bson:"methodDisplayName" json:"methodDisplayName,omitempty"`
	OwnerId           *primitive.ObjectID `bson:"ownerId" json:"ownerId,omitempty"`
	PaymentMethodId   *primitive.ObjectID `bson:"paymentMethodId" json:"paymentMethodId,omitempty"`
	ReferenceCode     *string             `bson:"referenceCode" json:"referenceCode,omitempty"`
	...
	PaymentMethod *registry_payment_method.MongoRecord `bson:"paymentMethod,omitempty" json:"paymentMethod,omitempty"`
	// Ref field Registry
...
func executeAggregation(ctx context.Context, where WhereClause, collection *mongo.Collection, options AggregateOptions) (AggregateQueryResult, error) {
	...
	// Build match stage
	matchStage := bson.D{}
	matchStage = append(matchStage, bson.E{Key: "$match", Value: whereCart})
	...
	// Add $lookup stage for PaymentMethod if projection is specified
	if options.PaymentMethodProjection != nil {
		objectProject := bson.E{Key: "$project", Value: options.PaymentMethodProjection.ToBson()}
		objectPipeline := bson.D{objectProject}

		pipeline = append(pipeline, bson.D{
			{Key: "$lookup", Value: bson.D{
				{Key: "from", Value: "registry_payment_methods"},
				{Key: "localField", Value: "paymentMethodId"},
				{Key: "foreignField", Value: "_id"},
