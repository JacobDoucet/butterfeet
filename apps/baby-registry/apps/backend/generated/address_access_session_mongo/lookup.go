...
type LookupOptions struct {
	Projection              address_access_session.Projection
	Sort                    address_access_session.MongoSortParams
...
type WhereClause struct {
	AddressAccessSession address_access_session.MongoWhereClause
	ApprovedGuest        registry_approved_guest.MongoWhereClause
...
func aggregateWithRefs(ctx context.Context, where WhereClause, collection *mongo.Collection, lookupOptions LookupOptions) (QueryResult, error) {
	...
	if lookupOptions.Sort.TokenHash > 0 {
		sortStage = append(sortStage, bson.E{Key: "tokenHash", Value: 1})
	} else if lookupOptions.Sort.TokenHash < 0 {
		sortStage = append(sortStage, bson.E{Key: "tokenHash", Value: -1})
	}
	...
	if lookupOptions.ApprovedGuestProjection != nil {
		lookupOptions.Projection.ApprovedGuestId = true
	}
	...
	dataPipeline := mongo.Pipeline{
		{
			{Key: "$match", Value: whereAddressAccessSession},
		},
		{
	...
	// Add $lookup stage for ApprovedGuest
	if lookupOptions.ApprovedGuestProjection != nil {
		// whereApprovedGuestId, err := where.ApprovedGuest.GetLookupQuery()
		// if err != nil {
		//     return QueryResult{}, err
		// }
		objectProject := bson.E{Key: "$project", Value: lookupOptions.ApprovedGuestProjection.ToBson()}
		objectPipeline := bson.D{objectProject}
		// if len(whereApprovedGuestId) > 0 {
		//     objectPipeline = bson.D{
		//         {Key: "$match", Value: whereApprovedGuestId},
		//         objectProject,
		//     }
		// }
		dataPipeline = append(dataPipeline, bson.D{
			{Key: "$lookup", Value: bson.D{
				...
				{Key: "localField", Value: "registryApprovedGuestId"},
			...
			Value: bson.D{
				...
				{Key: "count", Value: bson.A{
					bson.D{
						{
							Key:   "$match",
							Value: whereAddressAccessSession,
						},
					},
					bson.D{
						{
