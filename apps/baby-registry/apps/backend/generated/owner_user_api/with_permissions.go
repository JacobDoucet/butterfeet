...
type OnBeforeSearchHook func(ctx context.Context, actor permissions.Actor, query owner_user.WhereClause, options QueryOptions) (owner_user.WhereClause, QueryOptions, error)
type OnSearchHook func(ctx context.Context, actor permissions.Actor, r QueryResult, p Projection, err error) error
type OnBeforeSelectByIdHook func(ctx context.Context, actor permissions.Actor, query owner_user.SelectByIdQuery, projection Projection) (owner_user.SelectByIdQuery, Projection, error)
type OnSelectByIdHook func(ctx context.Context, actor permissions.Actor, m Model, p Projection, err error) error
type OnBeforeSelectByEmailUniqueHook func(ctx context.Context, actor permissions.Actor, query owner_user.SelectByEmailUniqueQuery, projection Projection) (owner_user.SelectByEmailUniqueQuery, Projection, error)
type OnSelectByEmailUniqueHook func(ctx context.Context, actor permissions.Actor, m Model, p Projection, err error) error
type OnBeforeCreateHook func(ctx context.Context, actor permissions.Actor, obj owner_user.Model, projection owner_user.Projection) (owner_user.Model, owner_user.Projection, error)
type OnCreateHook func(ctx context.Context, actor permissions.Actor, m owner_user.Model, p owner_user.Projection, err error) error
type OnBeforeUpdateHook func(ctx context.Context, actor permissions.Actor, obj owner_user.Model, projection owner_user.Projection) (owner_user.Model, owner_user.Projection, error)
type OnUpdateHook func(ctx context.Context, actor permissions.Actor, m owner_user.Model, p owner_user.Projection, err error) error
...
type Hooks struct {
	...
	OnBeforeSelectByEmailUnique OnBeforeSelectByEmailUniqueHook
	OnSelectByEmailUnique       OnSelectByEmailUniqueHook
...
func (c *clientWithPermissions) Search(ctx context.Context, actor permissions.Actor, query owner_user.WhereClause, options QueryOptions) (QueryResult, Projection, error) {
	...
	where, err := owner_user.ApplyActorReadPermissionsToWhereClause(actor, query)
	if err != nil {
		return QueryResult{}, Projection{}, err
	}
...
func (c *clientWithPermissions) SelectById(ctx context.Context, actor permissions.Actor, query owner_user.SelectByIdQuery, projection Projection) (Model, Projection, error) {
	for _, hook := range c.hooks {
	...
	result, resultProjection, err := c.Search(ctx, actor, owner_user.WhereClause{
		IdEq: &query.Id,
	}, QueryOptions{
	...
	if len(result.Data) == 0 {
		return Model{}, Projection{}, coded_error.NewNotFoundError("ownerUser")
	}
...
func (c *clientWithPermissions) SelectByEmailUnique(ctx context.Context, actor permissions.Actor, query owner_user.SelectByEmailUniqueQuery, projection Projection) (Model, Projection, error) {
	for _, hook := range c.hooks {
		if hook.OnBeforeSelectByEmailUnique != nil {
			var err error
			query, projection, err = hook.OnBeforeSelectByEmailUnique(ctx, actor, query, projection)
			if err != nil {
				return Model{}, Projection{}, err
			}
		}
	...
	for _, hook := range c.hooks {
		if hook.OnSelectByEmailUnique != nil {
			err = hook.OnSelectByEmailUnique(ctx, actor, result.Data[0], resultProjection, err)
			if err != nil {
				return Model{}, Projection{}, err
			}
		}
...
func (c *clientWithPermissions) CanWrite(ctx context.Context, actor permissions.Actor, obj owner_user.Model) error {
	ok := owner_user.HasWritePermissions(obj, actor)
	if !ok {
	...
	abacProjection := Projection{Projection: owner_user.GetAbacProjection(actor)}
	dbRecord, _, err := c.SelectById(ctx, actor, owner_user.SelectByIdQuery{Id: obj.Id}, abacProjection)
	if err != nil {
		return coded_error.NewNotFoundError("ownerUser")
	}
	ok = owner_user.HasWritePermissions(dbRecord.Model, actor)
	if !ok {
		return coded_error.NewUnauthorizedError("no write permissions")
	}
...
func ValidateBeforeCreate(ctx context.Context, actor permissions.Actor, obj owner_user.Model, projection owner_user.Projection) (owner_user.Model, owner_user.Projection, error) {
	var errs []string
	if !projection.Email {
	...
	if len(errs) > 0 {
		return owner_user.Model{},
			owner_user.Projection{},
			coded_error.NewInvalidRequestError("failed creation validation: " + strings.Join(errs, ", "))
	}
	return obj, projection, nil
}

func (c *clientWithPermissions) Create(ctx context.Context, actor permissions.Actor, obj owner_user.Model, projection owner_user.Projection) (owner_user.Model, owner_user.Projection, error) {
	var err error
	obj, projection, err = ValidateBeforeCreate(ctx, actor, obj, projection)
	if err != nil {
		return owner_user.Model{}, owner_user.Projection{}, err
	}
	...
	projection = owner_user.ProjectWritePermissions(projection, actor)
...
func (c *clientWithPermissions) Update(ctx context.Context, actor permissions.Actor, obj owner_user.Model, projection owner_user.Projection) (owner_user.Model, owner_user.Projection, error) {
	for _, hook := range c.hooks {
	...
	where := owner_user.WhereClause{}
	if obj.Id != "" {
		where.IdEq = &obj.Id
	}

	var err error
	where, err = owner_user.ApplyActorWritePermissionsToWhereClause(actor, where)
	if err != nil {
		return owner_user.Model{}, owner_user.Projection{}, err
	}
...
func (c *clientWithPermissions) CanDelete(ctx context.Context, actor permissions.Actor, id string) error {
	...
	dbRecord, _, err := c.SelectById(ctx, actor, owner_user.SelectByIdQuery{Id: id}, abacProjection)
	if err != nil {
		return coded_error.NewNotFoundError("ownerUser")
	}
	ok := owner_user.HasWritePermissions(dbRecord.Model, actor)
...
func (c *clientWithPermissions) PaginateAll(ctx context.Context, actor permissions.Actor, query owner_user.WhereClause, options PaginationOptions) (<-chan Model, <-chan error) {
