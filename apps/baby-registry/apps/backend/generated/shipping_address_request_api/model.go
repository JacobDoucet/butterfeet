...
type Client interface {
	Search(ctx context.Context, actor permissions.Actor, query shipping_address_request.WhereClause, options QueryOptions) (QueryResult, Projection, error)
	SelectById(ctx context.Context, actor permissions.Actor, query shipping_address_request.SelectByIdQuery, projection Projection) (Model, Projection, error)
	Create(ctx context.Context, actor permissions.Actor, obj shipping_address_request.Model, projection shipping_address_request.Projection) (shipping_address_request.Model, shipping_address_request.Projection, error)
	Update(ctx context.Context, actor permissions.Actor, obj shipping_address_request.Model, projection shipping_address_request.Projection) (shipping_address_request.Model, shipping_address_request.Projection, error)
	Delete(ctx context.Context, actor permissions.Actor, id string) error
	PaginateAll(ctx context.Context, actor permissions.Actor, query shipping_address_request.WhereClause, options PaginationOptions) (<-chan Model, <-chan error)
	Aggregate(ctx context.Context, actor permissions.Actor, query shipping_address_request.WhereClause, options AggregateOptions) (AggregateResult, error)
}

type clientImpl interface {
	Search(ctx context.Context, query WhereClause, options QueryOptions) (QueryResult, error)
	Create(ctx context.Context, obj shipping_address_request.Model, projection shipping_address_request.Projection) (shipping_address_request.Model, error)
	Update(ctx context.Context, obj shipping_address_request.Model, where shipping_address_request.WhereClause, projection shipping_address_request.Projection) (shipping_address_request.Model, error)
...
type Model struct {
	...
	RegistryItem *registry_item.Model
}

type WhereClause struct {
	ShippingAddressRequest shipping_address_request.WhereClause
...
type QueryOptions struct {
	...
	Sort       shipping_address_request.SortParams
	Limit      int
	Skip       int
}
...
type Projection struct {
	shipping_address_request.Projection `json:",inline"`
	Owner                               *owner_user.Projection    `json:"Owner,omitempty"`
	Registry                            *registry.Projection      `json:"Registry,omitempty"`
	RegistryItem                        *registry_item.Projection `json:"RegistryItem,omitempty"`
}
...
func NewProjection(defaultVal bool) Projection {
	...
	registryItemProjection := registry_item.NewProjection(defaultVal)
	return Projection{
		Projection:   shipping_address_request.NewProjection(defaultVal),
...
func projectReadPermissions(actor permissions.Actor, projection Projection) Projection {
	projection.Projection = shipping_address_request.ProjectReadPermissions(projection.Projection, actor)
	...
	if projection.RegistryItem != nil {
		registryItemProjection := registry_item.ProjectReadPermissions(*projection.RegistryItem, actor)
		projection.RegistryItem = &registryItemProjection
	}
...
func (m *Model) GetRegistryItem() registry_item.Model {
	if m.RegistryItem == nil {
...
// Valid aggregatable fields for ShippingAddressRequest
const (
	AggregateFieldPolicyVersion AggregateField = "policyVersion"
)

// ValidAggregateFields returns all valid aggregatable fields
func ValidAggregateFields() []AggregateField {
	return []AggregateField{
		AggregateFieldPolicyVersion,
	}
}
...
// Valid group-by fields for ShippingAddressRequest
const (
	GroupByFieldDecisionReason GroupByField = "decisionReason"
	GroupByFieldEmailEnc       GroupByField = "emailEnc"
	GroupByFieldEmailHash      GroupByField = "emailHash"
	GroupByFieldName           GroupByField = "name"
	GroupByFieldNote           GroupByField = "note"
	GroupByFieldOwnerId        GroupByField = "ownerId"
	GroupByFieldPolicyVersion  GroupByField = "policyVersion"
	GroupByFieldRegistryId     GroupByField = "registryId"
	GroupByFieldRegistryItemId GroupByField = "registryItemId"
)
...
func ValidGroupByFields() []GroupByField {
	return []GroupByField{
		...
		GroupByFieldEmailEnc,
		GroupByFieldEmailHash,
		GroupByFieldName,
		GroupByFieldNote,
		GroupByFieldOwnerId,
		GroupByFieldPolicyVersion,
		GroupByFieldRegistryId,
		GroupByFieldRegistryItemId,
	}
}
...
type AggregateOptions struct {
	...
	// Projection for RegistryItem ref field
...
type AggregateResultRow struct {
	...
	RegistryItem *registry_item.Model `json:"registryItem,omitempty"`
