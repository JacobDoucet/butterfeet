...
type OnBeforeSearchHook func(ctx context.Context, actor permissions.Actor, query event.WhereClause, options QueryOptions) (event.WhereClause, QueryOptions, error)
type OnSearchHook func(ctx context.Context, actor permissions.Actor, r QueryResult, p Projection, err error) error
type OnBeforeSelectByIdHook func(ctx context.Context, actor permissions.Actor, query event.SelectByIdQuery, projection Projection) (event.SelectByIdQuery, Projection, error)
type OnSelectByIdHook func(ctx context.Context, actor permissions.Actor, m Model, p Projection, err error) error
type OnBeforeCreateHook func(ctx context.Context, actor permissions.Actor, obj event.Model, projection event.Projection) (event.Model, event.Projection, error)
type OnCreateHook func(ctx context.Context, actor permissions.Actor, m event.Model, p event.Projection, err error) error
type OnBeforeUpdateHook func(ctx context.Context, actor permissions.Actor, obj event.Model, projection event.Projection) (event.Model, event.Projection, error)
type OnUpdateHook func(ctx context.Context, actor permissions.Actor, m event.Model, p event.Projection, err error) error
...
func (c *clientWithPermissions) Search(ctx context.Context, actor permissions.Actor, query event.WhereClause, options QueryOptions) (QueryResult, Projection, error) {
	...
	where, err := event.ApplyActorReadPermissionsToWhereClause(actor, query)
	if err != nil {
		return QueryResult{}, Projection{}, err
	}
...
func (c *clientWithPermissions) SelectById(ctx context.Context, actor permissions.Actor, query event.SelectByIdQuery, projection Projection) (Model, Projection, error) {
	for _, hook := range c.hooks {
	...
	result, resultProjection, err := c.Search(ctx, actor, event.WhereClause{
		IdEq: &query.Id,
	}, QueryOptions{
	...
	if len(result.Data) == 0 {
		return Model{}, Projection{}, coded_error.NewNotFoundError("event")
	}
...
func (c *clientWithPermissions) CanWrite(ctx context.Context, actor permissions.Actor, obj event.Model) error {
	ok := event.HasWritePermissions(obj, actor)
	if !ok {
	...
	abacProjection := Projection{Projection: event.GetAbacProjection(actor)}
	dbRecord, _, err := c.SelectById(ctx, actor, event.SelectByIdQuery{Id: obj.Id}, abacProjection)
	if err != nil {
		return coded_error.NewNotFoundError("event")
	}
	ok = event.HasWritePermissions(dbRecord.Model, actor)
	if !ok {
		return coded_error.NewUnauthorizedError("no write permissions")
	}
...
func (c *clientWithPermissions) Create(ctx context.Context, actor permissions.Actor, obj event.Model, projection event.Projection) (event.Model, event.Projection, error) {
	var err error
	...
	for _, hook := range c.hooks {
		if hook.OnBeforeCreate != nil {
			...
			if err != nil {
				return event.Model{}, event.Projection{}, err
			}
		}
	}
	...
	projection = event.ProjectWritePermissions(projection, actor)
...
func (c *clientWithPermissions) Update(ctx context.Context, actor permissions.Actor, obj event.Model, projection event.Projection) (event.Model, event.Projection, error) {
	for _, hook := range c.hooks {
	...
	var err error
	where, err = event.ApplyActorWritePermissionsToWhereClause(actor, where)
	if err != nil {
		return event.Model{}, event.Projection{}, err
	}
...
func (c *clientWithPermissions) CanDelete(ctx context.Context, actor permissions.Actor, id string) error {
	...
	dbRecord, _, err := c.SelectById(ctx, actor, event.SelectByIdQuery{Id: id}, abacProjection)
	if err != nil {
		return coded_error.NewNotFoundError("event")
	}
	ok := event.HasWritePermissions(dbRecord.Model, actor)
...
func (c *clientWithPermissions) PaginateAll(ctx context.Context, actor permissions.Actor, query event.WhereClause, options PaginationOptions) (<-chan Model, <-chan error) {
