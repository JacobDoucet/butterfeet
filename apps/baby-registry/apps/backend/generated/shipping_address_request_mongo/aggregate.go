...
type AggregateOptions struct {
	...
	RegistryItemProjection *registry_item.Projection
}
...
type AggregateResultRow struct {
	...
	EmailEnc       *string             `bson:"emailEnc" json:"emailEnc,omitempty"`
	...
	PolicyVersion  *int                `bson:"policyVersion" json:"policyVersion,omitempty"`
	RegistryId     *primitive.ObjectID `bson:"registryId" json:"registryId,omitempty"`
	RegistryItemId *primitive.ObjectID `bson:"registryItemId" json:"registryItemId,omitempty"`
	...
	RegistryItem *registry_item.MongoRecord `bson:"registryItem,omitempty" json:"registryItem,omitempty"`
	// Metadata
...
func executeAggregation(ctx context.Context, where WhereClause, collection *mongo.Collection, options AggregateOptions) (AggregateQueryResult, error) {
	whereShippingAddressRequest, err := where.ShippingAddressRequest.GetLookupQuery()
	if err != nil {
	...
	// Build match stage
	matchStage := bson.D{}
	matchStage = append(matchStage, bson.E{Key: "$match", Value: whereShippingAddressRequest})
	...
	// Add $lookup stage for RegistryItem if projection is specified
	if options.RegistryItemProjection != nil {
		objectProject := bson.E{Key: "$project", Value: options.RegistryItemProjection.ToBson()}
		objectPipeline := bson.D{objectProject}
