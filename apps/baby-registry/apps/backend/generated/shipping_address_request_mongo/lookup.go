...
type LookupOptions struct {
	Projection             shipping_address_request.Projection
	Sort                   shipping_address_request.MongoSortParams
...
type WhereClause struct {
	ShippingAddressRequest shipping_address_request.MongoWhereClause
...
func aggregateWithRefs(ctx context.Context, where WhereClause, collection *mongo.Collection, lookupOptions LookupOptions) (QueryResult, error) {
	...
	if lookupOptions.RegistryItemProjection != nil {
		lookupOptions.Projection.RegistryItemId = true
	}

	dataPipeline := mongo.Pipeline{
		{
			{Key: "$match", Value: whereShippingAddressRequest},
		},
		{
	...
	// Add $lookup stage for RegistryItem
	if lookupOptions.RegistryItemProjection != nil {
		// whereRegistryItemId, err := where.RegistryItem.GetLookupQuery()
		// if err != nil {
		//     return QueryResult{}, err
		// }
		objectProject := bson.E{Key: "$project", Value: lookupOptions.RegistryItemProjection.ToBson()}
		objectPipeline := bson.D{objectProject}
		// if len(whereRegistryItemId) > 0 {
		//     objectPipeline = bson.D{
		//         {Key: "$match", Value: whereRegistryItemId},
		//         objectProject,
		//     }
		// }
		dataPipeline = append(dataPipeline, bson.D{
			...
			Value: bson.D{
				...
				{Key: "count", Value: bson.A{
					bson.D{
						{
							Key:   "$match",
							Value: whereShippingAddressRequest,
						},
					},
					bson.D{
						{
