...
type OnBeforeSearchHook func(ctx context.Context, actor permissions.Actor, query registry_payment_method.WhereClause, options QueryOptions) (registry_payment_method.WhereClause, QueryOptions, error)
type OnSearchHook func(ctx context.Context, actor permissions.Actor, r QueryResult, p Projection, err error) error
type OnBeforeSelectByIdHook func(ctx context.Context, actor permissions.Actor, query registry_payment_method.SelectByIdQuery, projection Projection) (registry_payment_method.SelectByIdQuery, Projection, error)
type OnSelectByIdHook func(ctx context.Context, actor permissions.Actor, m Model, p Projection, err error) error
type OnBeforeCreateHook func(ctx context.Context, actor permissions.Actor, obj registry_payment_method.Model, projection registry_payment_method.Projection) (registry_payment_method.Model, registry_payment_method.Projection, error)
type OnCreateHook func(ctx context.Context, actor permissions.Actor, m registry_payment_method.Model, p registry_payment_method.Projection, err error) error
type OnBeforeUpdateHook func(ctx context.Context, actor permissions.Actor, obj registry_payment_method.Model, projection registry_payment_method.Projection) (registry_payment_method.Model, registry_payment_method.Projection, error)
type OnUpdateHook func(ctx context.Context, actor permissions.Actor, m registry_payment_method.Model, p registry_payment_method.Projection, err error) error
...
func (c *clientWithPermissions) Search(ctx context.Context, actor permissions.Actor, query registry_payment_method.WhereClause, options QueryOptions) (QueryResult, Projection, error) {
	...
	where, err := registry_payment_method.ApplyActorReadPermissionsToWhereClause(actor, query)
	if err != nil {
		return QueryResult{}, Projection{}, err
	}
...
func (c *clientWithPermissions) SelectById(ctx context.Context, actor permissions.Actor, query registry_payment_method.SelectByIdQuery, projection Projection) (Model, Projection, error) {
	for _, hook := range c.hooks {
	...
	result, resultProjection, err := c.Search(ctx, actor, registry_payment_method.WhereClause{
		IdEq: &query.Id,
	}, QueryOptions{
	...
	if len(result.Data) == 0 {
		return Model{}, Projection{}, coded_error.NewNotFoundError("registryPaymentMethod")
	}
...
func (c *clientWithPermissions) CanWrite(ctx context.Context, actor permissions.Actor, obj registry_payment_method.Model) error {
	ok := registry_payment_method.HasWritePermissions(obj, actor)
	if !ok {
	...
	abacProjection := Projection{Projection: registry_payment_method.GetAbacProjection(actor)}
	dbRecord, _, err := c.SelectById(ctx, actor, registry_payment_method.SelectByIdQuery{Id: obj.Id}, abacProjection)
	if err != nil {
		return coded_error.NewNotFoundError("registryPaymentMethod")
	}
	ok = registry_payment_method.HasWritePermissions(dbRecord.Model, actor)
	if !ok {
		return coded_error.NewUnauthorizedError("no write permissions")
	}
...
func ValidateBeforeCreate(ctx context.Context, actor permissions.Actor, obj registry_payment_method.Model, projection registry_payment_method.Projection) (registry_payment_method.Model, registry_payment_method.Projection, error) {
	var errs []string
	...
	} else if valErr := enum_payment_method_type.Validate(obj.Type); valErr != nil {
		errs = append(errs, "Type must be a valid PaymentMethodType")
	}
	if len(errs) > 0 {
		return registry_payment_method.Model{},
			registry_payment_method.Projection{},
			coded_error.NewInvalidRequestError("failed creation validation: " + strings.Join(errs, ", "))
	}
	return obj, projection, nil
}

func (c *clientWithPermissions) Create(ctx context.Context, actor permissions.Actor, obj registry_payment_method.Model, projection registry_payment_method.Projection) (registry_payment_method.Model, registry_payment_method.Projection, error) {
	var err error
	obj, projection, err = ValidateBeforeCreate(ctx, actor, obj, projection)
	if err != nil {
		return registry_payment_method.Model{}, registry_payment_method.Projection{}, err
	}
	...
	projection = registry_payment_method.ProjectWritePermissions(projection, actor)
...
func (c *clientWithPermissions) Update(ctx context.Context, actor permissions.Actor, obj registry_payment_method.Model, projection registry_payment_method.Projection) (registry_payment_method.Model, registry_payment_method.Projection, error) {
	for _, hook := range c.hooks {
	...
	where := registry_payment_method.WhereClause{}
	if obj.Id != "" {
		where.IdEq = &obj.Id
	}

	var err error
	where, err = registry_payment_method.ApplyActorWritePermissionsToWhereClause(actor, where)
	if err != nil {
		return registry_payment_method.Model{}, registry_payment_method.Projection{}, err
	}
...
func (c *clientWithPermissions) CanDelete(ctx context.Context, actor permissions.Actor, id string) error {
	...
	dbRecord, _, err := c.SelectById(ctx, actor, registry_payment_method.SelectByIdQuery{Id: id}, abacProjection)
	if err != nil {
		return coded_error.NewNotFoundError("registryPaymentMethod")
	}
	ok := registry_payment_method.HasWritePermissions(dbRecord.Model, actor)
...
func (c *clientWithPermissions) PaginateAll(ctx context.Context, actor permissions.Actor, query registry_payment_method.WhereClause, options PaginationOptions) (<-chan Model, <-chan error) {
...
func (c *clientWithPermissions) Aggregate(ctx context.Context, actor permissions.Actor, query registry_payment_method.WhereClause, options AggregateOptions) (AggregateResult, error) {
