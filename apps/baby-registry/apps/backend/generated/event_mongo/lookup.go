...
func aggregateWithRefs(ctx context.Context, where WhereClause, collection *mongo.Collection, lookupOptions LookupOptions) (QueryResult, error) {
	whereEvent, err := where.Event.GetLookupQuery()
	if err != nil {
	...
	if lookupOptions.Sort.SubjectsSubjectId > 0 {
		sortStage = append(sortStage, bson.E{Key: "subjects.subjectId", Value: 1})
	} else if lookupOptions.Sort.SubjectsSubjectId < 0 {
		sortStage = append(sortStage, bson.E{Key: "subjects.subjectId", Value: -1})
	}
	if lookupOptions.Sort.Type > 0 {
		sortStage = append(sortStage, bson.E{Key: "type", Value: 1})
	} else if lookupOptions.Sort.Type < 0 {
		sortStage = append(sortStage, bson.E{Key: "type", Value: -1})
	}
