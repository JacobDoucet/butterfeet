...
func RegisterRoutes(handlerProps HandlerProps) (*http.ServeMux, error) {
	...
	selectByReferenceUniqueHandler, err := GetSelectByReferenceUniqueHandler(handlerProps)
	if err != nil {
		return nil, err
	}
	routes.HandleFunc("/referenceCode/{referenceCode}", selectByReferenceUniqueHandler)
