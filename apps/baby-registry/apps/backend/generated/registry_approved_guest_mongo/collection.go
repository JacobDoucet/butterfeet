...
const CollectionName = "registry_approved_guests"
...
func CreateIndexes(ctx context.Context, db *mongo.Database) error {
	...
	if err := createRegistryEmailUniqueIndex(ctx, db); err != nil {
		return err
	}
...
func createRegistryEmailUniqueIndex(ctx context.Context, db *mongo.Database) error {
	collection := db.Collection(CollectionName)
	...
	_, err := collection.Indexes().CreateOne(ctx, mongo.IndexModel{
		...
		Options: options.Index().SetName("registry_email_unique").SetUnique(true),
	})
	return err
}
