...
type LookupOptions struct {
	Projection         registry_payment_method.Projection
	Sort               registry_payment_method.MongoSortParams
...
type WhereClause struct {
	RegistryPaymentMethod registry_payment_method.MongoWhereClause
...
func aggregateWithRefs(ctx context.Context, where WhereClause, collection *mongo.Collection, lookupOptions LookupOptions) (QueryResult, error) {
	...
	dataPipeline := mongo.Pipeline{
		{
			{Key: "$match", Value: whereRegistryPaymentMethod},
		},
		{
			...
			Value: bson.D{
				...
				{Key: "count", Value: bson.A{
					bson.D{
						{
							Key:   "$match",
							Value: whereRegistryPaymentMethod,
						},
					},
					bson.D{
						{
