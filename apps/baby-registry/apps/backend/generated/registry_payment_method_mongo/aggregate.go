...
type AggregateResultRow struct {
	...
	BankAccountName   *string             `bson:"bankAccountName" json:"bankAccountName,omitempty"`
	BankAccountNumber *string             `bson:"bankAccountNumber" json:"bankAccountNumber,omitempty"`
	BankIban          *string             `bson:"bankIban" json:"bankIban,omitempty"`
	BankName          *string             `bson:"bankName" json:"bankName,omitempty"`
	BankRoutingNumber *string             `bson:"bankRoutingNumber" json:"bankRoutingNumber,omitempty"`
	BankSwift         *string             `bson:"bankSwift" json:"bankSwift,omitempty"`
	...
	PaymentUrl        *string             `bson:"paymentUrl" json:"paymentUrl,omitempty"`
	Position          *int                `bson:"position" json:"position,omitempty"`
	RecipientEmail    *string             `bson:"recipientEmail" json:"recipientEmail,omitempty"`
	RecipientPhone    *string             `bson:"recipientPhone" json:"recipientPhone,omitempty"`
...
func executeAggregation(ctx context.Context, where WhereClause, collection *mongo.Collection, options AggregateOptions) (AggregateQueryResult, error) {
	whereRegistryPaymentMethod, err := where.RegistryPaymentMethod.GetLookupQuery()
	if err != nil {
	...
	// Build match stage
	matchStage := bson.D{}
	matchStage = append(matchStage, bson.E{Key: "$match", Value: whereRegistryPaymentMethod})
	...
	if options.CartsProjection != nil {
		...
		pipeline = append(pipeline, bson.D{
			{Key: "$lookup", Value: bson.D{
				...
				{Key: "foreignField", Value: "registryPaymentMethodId"},
				{Key: "as", Value: "carts"},
				{Key: "pipeline", Value: bson.A{objectPipeline}},
			}},
		})
