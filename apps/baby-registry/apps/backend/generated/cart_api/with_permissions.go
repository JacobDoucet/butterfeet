...
type OnBeforeSearchHook func(ctx context.Context, actor permissions.Actor, query cart.WhereClause, options QueryOptions) (cart.WhereClause, QueryOptions, error)
type OnSearchHook func(ctx context.Context, actor permissions.Actor, r QueryResult, p Projection, err error) error
type OnBeforeSelectByIdHook func(ctx context.Context, actor permissions.Actor, query cart.SelectByIdQuery, projection Projection) (cart.SelectByIdQuery, Projection, error)
type OnSelectByIdHook func(ctx context.Context, actor permissions.Actor, m Model, p Projection, err error) error
type OnBeforeSelectByReferenceUniqueHook func(ctx context.Context, actor permissions.Actor, query cart.SelectByReferenceUniqueQuery, projection Projection) (cart.SelectByReferenceUniqueQuery, Projection, error)
type OnSelectByReferenceUniqueHook func(ctx context.Context, actor permissions.Actor, m Model, p Projection, err error) error
type OnBeforeCreateHook func(ctx context.Context, actor permissions.Actor, obj cart.Model, projection cart.Projection) (cart.Model, cart.Projection, error)
type OnCreateHook func(ctx context.Context, actor permissions.Actor, m cart.Model, p cart.Projection, err error) error
type OnBeforeUpdateHook func(ctx context.Context, actor permissions.Actor, obj cart.Model, projection cart.Projection) (cart.Model, cart.Projection, error)
type OnUpdateHook func(ctx context.Context, actor permissions.Actor, m cart.Model, p cart.Projection, err error) error
...
type Hooks struct {
	...
	OnBeforeSelectByReferenceUnique OnBeforeSelectByReferenceUniqueHook
	OnSelectByReferenceUnique       OnSelectByReferenceUniqueHook
...
func (c *clientWithPermissions) Search(ctx context.Context, actor permissions.Actor, query cart.WhereClause, options QueryOptions) (QueryResult, Projection, error) {
	...
	where, err := cart.ApplyActorReadPermissionsToWhereClause(actor, query)
	if err != nil {
		return QueryResult{}, Projection{}, err
	}
	...
	result, err := c.client.Search(ctx, WhereClause{
		...
		PaymentMethod: whereRegistryPaymentMethod,
...
func (c *clientWithPermissions) SelectById(ctx context.Context, actor permissions.Actor, query cart.SelectByIdQuery, projection Projection) (Model, Projection, error) {
	for _, hook := range c.hooks {
	...
	result, resultProjection, err := c.Search(ctx, actor, cart.WhereClause{
		IdEq: &query.Id,
	}, QueryOptions{
	...
	if len(result.Data) == 0 {
		return Model{}, Projection{}, coded_error.NewNotFoundError("cart")
	}
...
func (c *clientWithPermissions) SelectByReferenceUnique(ctx context.Context, actor permissions.Actor, query cart.SelectByReferenceUniqueQuery, projection Projection) (Model, Projection, error) {
	for _, hook := range c.hooks {
		if hook.OnBeforeSelectByReferenceUnique != nil {
			var err error
			query, projection, err = hook.OnBeforeSelectByReferenceUnique(ctx, actor, query, projection)
			if err != nil {
				return Model{}, Projection{}, err
			}
		}
	...
	result, resultProjection, err := c.Search(ctx, actor, cart.WhereClause{
		ReferenceCodeEq: &query.ReferenceCode,
	}, QueryOptions{
		Projection: &projection,
		Limit:      1,
	})
	...
	for _, hook := range c.hooks {
		if hook.OnSelectByReferenceUnique != nil {
			err = hook.OnSelectByReferenceUnique(ctx, actor, result.Data[0], resultProjection, err)
			if err != nil {
				return Model{}, Projection{}, err
			}
		}
...
func (c *clientWithPermissions) CanWrite(ctx context.Context, actor permissions.Actor, obj cart.Model) error {
	ok := cart.HasWritePermissions(obj, actor)
	if !ok {
	...
	abacProjection := Projection{Projection: cart.GetAbacProjection(actor)}
	dbRecord, _, err := c.SelectById(ctx, actor, cart.SelectByIdQuery{Id: obj.Id}, abacProjection)
	if err != nil {
		return coded_error.NewNotFoundError("cart")
	}
	ok = cart.HasWritePermissions(dbRecord.Model, actor)
	if !ok {
		return coded_error.NewUnauthorizedError("no write permissions")
	}
...
func ValidateBeforeCreate(ctx context.Context, actor permissions.Actor, obj cart.Model, projection cart.Projection) (cart.Model, cart.Projection, error) {
	var errs []string
	if !projection.OwnerId {
		errs = append(errs, "missing required field ownerId")
	}
	if !projection.ReferenceCode {
		errs = append(errs, "missing required field referenceCode")
	} else if obj.ReferenceCode == "" {
		errs = append(errs, "ReferenceCode cannot be empty")
	}
	...
	} else if valErr := enum_cart_status.Validate(obj.Status); valErr != nil {
		errs = append(errs, "Status must be a valid CartStatus")
	}
	if len(errs) > 0 {
		return cart.Model{},
			cart.Projection{},
...
func (c *clientWithPermissions) Create(ctx context.Context, actor permissions.Actor, obj cart.Model, projection cart.Projection) (cart.Model, cart.Projection, error) {
	var err error
	obj, projection, err = ValidateBeforeCreate(ctx, actor, obj, projection)
	if err != nil {
		return cart.Model{}, cart.Projection{}, err
	}
	...
	projection = cart.ProjectWritePermissions(projection, actor)
...
func (c *clientWithPermissions) Update(ctx context.Context, actor permissions.Actor, obj cart.Model, projection cart.Projection) (cart.Model, cart.Projection, error) {
	for _, hook := range c.hooks {
	...
	where := cart.WhereClause{}
	if obj.Id != "" {
		where.IdEq = &obj.Id
	}

	var err error
	where, err = cart.ApplyActorWritePermissionsToWhereClause(actor, where)
	if err != nil {
		return cart.Model{}, cart.Projection{}, err
	}
...
func (c *clientWithPermissions) CanDelete(ctx context.Context, actor permissions.Actor, id string) error {
	...
	dbRecord, _, err := c.SelectById(ctx, actor, cart.SelectByIdQuery{Id: id}, abacProjection)
	if err != nil {
		return coded_error.NewNotFoundError("cart")
	}
	ok := cart.HasWritePermissions(dbRecord.Model, actor)
...
func (c *clientWithPermissions) PaginateAll(ctx context.Context, actor permissions.Actor, query cart.WhereClause, options PaginationOptions) (<-chan Model, <-chan error) {
...
func (c *clientWithPermissions) Aggregate(ctx context.Context, actor permissions.Actor, query cart.WhereClause, options AggregateOptions) (AggregateResult, error) {
