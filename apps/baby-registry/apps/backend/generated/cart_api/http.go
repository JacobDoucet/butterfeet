...
func ToHTTPMutationResult(obj cart.Model, projection cart.Projection) (HTTPMutationResult, error) {
...
func ToHTTPModel(r Model, projection Projection) (HTTPModel, error) {
	...
	if r.PaymentMethod != nil && projection.PaymentMethod != nil {
		refProjection := *projection.PaymentMethod
		val, toHTTPRecordErr := r.PaymentMethod.ToHTTPRecord(refProjection)
		if toHTTPRecordErr != nil {
			err = errors.Join(err, toHTTPRecordErr)
		}
		m.PaymentMethod = &val
