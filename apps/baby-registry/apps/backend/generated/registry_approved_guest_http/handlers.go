...
type GetMetadataOnSearchHook func(ctx context.Context, actor permissions.Actor, queryResult registry_approved_guest_api.HTTPQueryResult) (map[string]any, error)
type GetMetadataOnCreateHook func(ctx context.Context, actor permissions.Actor, mutationResult registry_approved_guest_api.HTTPMutationResult) (map[string]any, error)
type GetMetadataOnUpdateHook func(ctx context.Context, actor permissions.Actor, mutationResult registry_approved_guest_api.HTTPMutationResult) (map[string]any, error)
type GetMetadataOnDeleteHook func(ctx context.Context, actor permissions.Actor, deleteResult registry_approved_guest_api.HTTPDeleteResult) (map[string]any, error)
...
type HandlerProps struct {
	Api           registry_approved_guest_api.Client
...
func (p *HandlerProps) onError(endpoint string, e error) {
	if p.OnError != nil {
		p.OnError(endpoint+"<RegistryApprovedGuest>", e)
	}
}

type SearchRequest struct {
	Query      registry_approved_guest.HTTPWhereClause `json:"query"`
	Sort       registry_approved_guest.HTTPSortParams  `json:"sort"`
	Projection *registry_approved_guest_api.Projection `json:"projection,omitempty"`
	Limit      int                                     `json:"limit,omitempty"`
	Skip       int                                     `json:"skip,omitempty"`
}

func (sr *SearchRequest) ResolveProjection() *registry_approved_guest_api.Projection {
	if sr.Projection != nil {
		return sr.Projection
	}
	projection := registry_approved_guest_api.NewProjection(true)
...
func GetSearchHandler(props HandlerProps) (http.HandlerFunc, error) {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		queryResult, projectionResult, err := props.Api.Search(ctx, actor, searchQuery, registry_approved_guest_api.QueryOptions{
		...
		response, err := registry_approved_guest_api.ToHTTPQueryResult(queryResult, projectionResult)
		if err != nil {
			props.onError("Search", err)
...
func (sr *SelectRequest) ResolveProjection() registry_approved_guest_api.Projection {
	if sr.Projection != nil {
		return *sr.Projection
	}
	return registry_approved_guest_api.NewProjection(true)
}
...
type SelectByIdRequest struct {
	Query registry_approved_guest.HTTPSelectByIdQuery `json:"query"`
}
...
func GetSelectByIdHandler(props HandlerProps) (http.HandlerFunc, error) {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		selectByIdRequest := SelectByIdRequest{
			Query: registry_approved_guest.HTTPSelectByIdQuery{
				Id: id,
			},
		}
		...
		response, err := registry_approved_guest_api.ToHTTPModel(getByIdResult, projectionResult)
		if err != nil {
			props.onError("SelectById", err)
			log.Debug().Err(err).Msg("failed to convert query result to HTTP response")
			http.Error(w, coded_error.ResolveErrorCodeAsString(err), coded_error.ResolveHTTPStatus(err))
...
type SelectByRegistryEmailUniqueRequest struct {
	Query registry_approved_guest.HTTPSelectByRegistryEmailUniqueQuery `json:"query"`
}

func GetSelectByRegistryEmailUniqueHandler(props HandlerProps) (http.HandlerFunc, error) {
	if err := props.Validate(); err != nil {
		return nil, err
	}
	return func(w http.ResponseWriter, r *http.Request) {
		selectRequest, err := resolveSelectRequest(r)
		if err != nil {
			props.onError("SelectByRegistryEmailUnique", err)
			log.Debug().Err(err).Msg("failed to resolve select request")
			http.Error(w, coded_error.ResolveErrorCodeAsString(err), coded_error.ResolveHTTPStatus(err))
			return
		}
		...
		if registryId == "" {
			props.onError("SelectByRegistryId", errors.New("missing path parameter: registryId"))
			log.Debug().Err(err).Msg("missing path parameter")
			http.Error(w, "missing query parameter: registryId", http.StatusBadRequest)
			return
		}

		emailHash := r.PathValue("emailHash")
		if emailHash == "" {
			props.onError("SelectByEmailHash", errors.New("missing path parameter: emailHash"))
			log.Debug().Err(err).Msg("missing path parameter")
			http.Error(w, "missing query parameter: emailHash", http.StatusBadRequest)
			return
		}

		selectByRegistryEmailUniqueRequest := SelectByRegistryEmailUniqueRequest{
			Query: registry_approved_guest.HTTPSelectByRegistryEmailUniqueQuery{
				RegistryId: registryId,
				EmailHash:  emailHash,
			},
		}

		selectByRegistryEmailUniqueQuery, err := selectByRegistryEmailUniqueRequest.Query.ToSelectByRegistryEmailUniqueQuery()
		if err != nil {
			props.onError("SelectByRegistryEmailUnique", err)
			log.Debug().Err(err).Msg("failed to resolve select query")
			http.Error(w, coded_error.ResolveErrorCodeAsString(err), coded_error.ResolveHTTPStatus(err))
		...
		getByRegistryEmailUniqueResult, projectionResult, err := props.Api.SelectByRegistryEmailUnique(ctx, actor, selectByRegistryEmailUniqueQuery, projection)
		if err != nil {
			props.onError("SelectByRegistryEmailUnique", err)
			log.Debug().Err(err).Msg("failed to execute select query")
			http.Error(w, coded_error.ResolveErrorCodeAsString(err), coded_error.ResolveHTTPStatus(err))
		...
		response, err := registry_approved_guest_api.ToHTTPModel(getByRegistryEmailUniqueResult, projectionResult)
		if err != nil {
			props.onError("SelectByRegistryEmailUnique", err)
			log.Debug().Err(err).Msg("failed to convert query result to HTTP response")
			http.Error(w, coded_error.ResolveErrorCodeAsString(err), coded_error.ResolveHTTPStatus(err))
...
func GetCreateHandler(props HandlerProps) (http.HandlerFunc, error) {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		projection := registry_approved_guest.NewProjection(true)
		projection.Id = false
		...
		response, err := registry_approved_guest_api.ToHTTPMutationResult(createResult, projectionResult)
		if err != nil {
			props.onError("Create", err)
...
func GetUpdateHandler(props HandlerProps) (http.HandlerFunc, error) {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		response, err := registry_approved_guest_api.ToHTTPMutationResult(updateResult, projectionResult)
		if err != nil {
			props.onError("Update", err)
...
func GetDeleteHandler(props HandlerProps) (http.HandlerFunc, error) {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		response := registry_approved_guest_api.ToHTTPDeleteResult(id)
...
func resolveSearchRequest(r *http.Request) (SearchRequest, error) {
	...
	case http.MethodGet:
		searchRequest := SearchRequest{
			Query: registry_approved_guest.HTTPWhereClause{},
		}
